...
if (!isNull _source && {_source != _unit}) then {
    if (side _source isEqualTo west) then {
        if (currentWeapon _source in ["hgun_P07_snds_F","arifle_SDAR_F"] && _projectile in ["B_9x21_Ball","B_9x21_Ball_Tracer_Red","B_9x21_Ball_Tracer_Green","B_9x21_Ball_Tracer_Yellow","B_556x45_dual"]) then {
            if (alive _unit) then {
